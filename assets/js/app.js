(function () {
  ...
  function currentRoute() {
    ...
    if (raw) return raw;
    // Cold start: putar video pembuka sekali per sesi (seperti aplikasi asli),
    // lalu lanjut ke landing. Tombol "Lewati" / ketuk = skip.
    return sessionStorage.getItem("ownertech-intro-played") ? "splash" : "startup";
  }
  ...
  function render() {
    ...
    if (route === "startup") return renderStartup();
  ...
  // ---------------------------------------------------------------- PUBLIC PAGES
  // Video pembuka cold-start (meniru screens/splash/startup_splash_screen.dart):
  // full-screen cover, latar emerald gelap, tombol "Lewati", ketuk = skip.
  // Video dipilih per orientasi — sama seperti app memilih per platform
  // (Win_startup = landscape/desktop, Andro_startup = portrait/HP).
  function renderStartup() {
    finishIntro.done = false;
    const portrait = window.matchMedia("(orientation: portrait)").matches;
    const src = portrait
      ? "assets/startup/Andro_startup.mp4"
      : "assets/startup/Win_startup.mp4";
    app.innerHTML = `
      <main class="startup-layout">
        <video class="startup-video" autoplay muted playsinline preload="auto">
          <source src="${src}" type="video/mp4">
        </video>
        <button class="startup-skip" data-action="finishIntro()">Lewati ${icon("chevron_right", "s18")}</button>
      </main>`;
    const video = app.querySelector(".startup-video");
    // Ketuk video di mana saja untuk lewati (seperti GestureDetector di app).
    video.addEventListener("click", finishIntro);
    // Lanjut otomatis saat video selesai atau bila gagal diputar.
    video.addEventListener("ended", finishIntro);
    video.addEventListener("error", finishIntro);
    // Failsafe: jangan terjebak bila 'ended' tak pernah datang.
    clearTimeout(renderStartup.failsafe);
    renderStartup.failsafe = setTimeout(finishIntro, 20000);
  ...
  function finishIntro() {
    clearTimeout(renderStartup.failsafe);
    if (finishIntro.done) return;
    finishIntro.done = true;
    sessionStorage.setItem("ownertech-intro-played", "yes");
    go("splash");
  }
  window.finishIntro = finishIntro;

  function replayIntro() {
    sessionStorage.removeItem("ownertech-intro-played");
    finishIntro.done = false;
    if (currentRoute() === "startup") renderStartup();
    else go("startup");
  }
    ...
    app.innerHTML = `
      <main class="splash-layout">
        <section class="splash-card">
          ...
          <div class="auth-center"><button class="link" data-action="replayIntro()" style="border:0;background:transparent">${icon("replay", "s16")} Putar ulang video pembuka</button></div>
        </section>
      </main>`;
    bindActions();
  }
  ...
  function callAction(action, el) {
    ...
    if (action === "replayIntro()") return replayIntro();
    if (action === "finishIntro()") return finishIntro();
